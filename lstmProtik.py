...
inp_data = genfromtxt('/home/protikbiswas100/pushGit/inp.csv', delimiter=',')
out_data = genfromtxt('/home/protikbiswas100/pushGit/mod_out.csv', delimiter=',')
out_data = np.reshape(out_data,(198,20))
print(out_data.shape)
out_data= np.array(out_data).tolist()
#print(out_data.shape)
#print(inp_data)
...
if len(sys.argv) == 1:
  ...
  sys.argv = ["", "0.5", "0.5", "0.5", "0.5", "1e-6", "15", "100"]#..............changed maxlen from 200 to 100
args = [float(a) for a in sys.argv[1:]]
print(args)
p_W, p_U, p_dense, p_emb, weight_decay, batch_size, maxlen = args
batch_size = int(batch_size)
maxlen = int(maxlen)
folder = "/home/protikbiswas100/pushGit/"
...
standard_prob = model.predict(X_train, batch_size=15, verbose=1)
A=np.asarray(mean_y_train + std_y_train * np.atleast_2d(Y_train).T)
B=np.asarray(mean_y_train + std_y_train * standard_prob)
A=A.reshape(B.shape)
print(np.mean((A
               - B)**2, 0)**0.5)
...
A=np.asarray(np.atleast_2d(Y_test).T)
B=np.asarray(mean_y_train + std_y_train * standard_prob)
A=A.reshape(B.shape)
print(np.mean((A - (B))**2, 0)**0.5)
A=np.asarray(np.atleast_2d(Y_test).T)
B=np.asarray(mean_y_train + std_y_train * prob_mean)
A=A.reshape(B.shape)
print(np.mean((A - (B))**2, 0)**0.5)
